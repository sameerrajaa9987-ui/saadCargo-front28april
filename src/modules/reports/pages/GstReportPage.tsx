...
export function GstReportPage() {
  ...
  const STATUS_COLORS: Record<string, string> = {
    draft: "text-slate-500", sent: "text-blue-600", paid: "text-green-600", cancelled: "text-red-500",
  };
  ...
  return (
    <div className="erp-page">
      <div className="flex items-center justify-between">
        ...
        <a href={getExportUrl("gst", { month, year })} target="_blank" rel="noopener noreferrer"
      ...
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
        <div className="flex flex-wrap gap-4 items-end">
          <div>
            ...
            <select value={month} onChange={(e) => setMonth(Number(e.target.value))} className="rounded-lg border border-input bg-background px-3 py-2 text-sm cursor-pointer focus:outline-none focus:ring-2 focus:ring-ring">
          ...
          <div>
            ...
            <select value={year} onChange={(e) => setYear(Number(e.target.value))} className="rounded-lg border border-input bg-background px-3 py-2 text-sm cursor-pointer focus:outline-none focus:ring-2 focus:ring-ring">
      ...
      <div className="overflow-auto rounded-xl border border-border bg-card shadow-sm">
        <table className="w-full text-sm min-w-[1100px]">
          <thead className="border-b border-border bg-muted/40">
            <tr>
              {["Bill No.", "Date", "Party", "GSTIN", "Status", "Reimb. (₹)", "Service (₹)", "CGST", "SGST", "IGST", "Gross (₹)"].map((h) => (
                <th key={h} className="px-4 py-3 text-left font-medium text-muted-foreground whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {isLoading && <tr><td colSpan={11} className="px-4 py-10 text-center"><div className="flex justify-center"><div className="h-6 w-6 animate-spin rounded-full border-2 border-primary border-t-transparent" /></div></td></tr>}
            {!isLoading && !data?.rows.length && <tr><td colSpan={11} className="px-4 py-10 text-center text-muted-foreground">No invoices for this period</td></tr>}
            {data?.rows.map((row) => (
              <tr key={row.billNumber} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-2.5"><span className={`capitalize font-medium ${STATUS_COLORS[row.status] ?? ""}`}>{row.status}</span></td>
                ...
                <td className="px-4 py-2.5">{row.cgstRate}% = {formatCurrency(row.cgstAmount)}</td>
                <td className="px-4 py-2.5">{row.sgstRate}% = {formatCurrency(row.sgstAmount)}</td>
                <td className="px-4 py-2.5">{row.igstRate}% = {formatCurrency(row.igstAmount)}</td>
