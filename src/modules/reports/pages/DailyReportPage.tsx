...
import { getDailyReport, downloadReportExport } from "../api/reportsApi";
        ...
        <button
          onClick={() =>
            downloadReportExport(
              "daily",
              { startDate, endDate },
              `daily-report_${startDate}_to_${endDate}.xlsx`,
            )
          }
          ...
          <tbody className="divide-y divide-border">
            ...
            {data?.map((row) => (
              <tr key={row.date} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-2.5">{formatCurrency(row.totalReimbursement)}</td>
                <td className="px-4 py-2.5">{formatCurrency(row.totalHamali)}</td>
                <td className="px-4 py-2.5">{formatCurrency(row.totalOther)}</td>
