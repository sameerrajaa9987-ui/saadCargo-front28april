  ...
  return (
    <ResourceListPage<Invoice, InvoiceListQuery>
      ...
      columns={[
        {
          header: "Bill No.",
          getValue: (inv) => <span className="font-mono font-semibold">{inv.billNumber}</span>,
        },
        { header: "Date", getValue: (inv) => formatDate(inv.date) },
        {
          header: "Party",
          getValue: (inv) => <span className="font-medium">{partyName(inv.party)}</span>,
        },
        {
          header: "Status",
          getValue: (inv) => (
        ...
        { header: "SGST (₹)", getValue: (inv) => formatCurrency(inv.sgstAmount) },
        {
          header: "Gross Total (₹)",
          getValue: (inv) => (
            <span className="font-semibold">{formatCurrency(inv.grossTotal)}</span>
          ),
        },
      ]}
        ...
        <div className="flex flex-wrap items-center gap-1">
          {inv.status === "draft" && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-md bg-blue-600 px-2 py-1.5 text-xs text-white hover:bg-blue-500 transition-colors disabled:opacity-50"
              >
          ...
          {inv.status === "sent" && (
            <button
              ...
              className="flex items-center gap-1 rounded-md bg-green-600 px-2 py-1.5 text-xs text-white hover:bg-green-500 transition-colors disabled:opacity-50"
            >
          ...
          {(inv.status === "draft" || inv.status === "sent") && (
            <button
              ...
              className="flex items-center gap-1 rounded-md border border-destructive/30 bg-destructive/10 px-2 py-1.5 text-xs text-destructive hover:bg-destructive/20 transition-colors"
            >
          ...
          <button
            onClick={() =>
              openAuthenticatedPdf(getInvoicePdfPath(inv.id), {
                filename: `invoice-${inv.billNumber}.pdf`,
              })
            }
            className="flex items-center gap-1 rounded-md border border-border bg-background px-2 py-1.5 text-xs hover:bg-accent transition-colors"
